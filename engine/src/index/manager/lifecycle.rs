...
impl super::IndexManager {
    ...
    fn reconcile_absent_tenant_deletion(&self, tenant_id: &TenantId, path: &Path) -> Result<()> {
        ...
        Err(FlapjackError::TenantNotFound(tenant_id.to_string()))
    }
    ...
    pub async fn delete_tenant(&self, tenant_id: &TenantId) -> Result<()> {
        ...
        let path = self.base_path.join(tenant_id);
        if !path.exists() {
            return self.reconcile_absent_tenant_deletion(tenant_id, &path);
        }
