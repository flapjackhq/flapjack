...
async fn activation_fixture_old_new_and_control_indexes_reopen_through_index_manager() {
    ...
    for (base, tenant, kind, query) in fixture_generations {
        let tenant_path = base.join(tenant);
        assert_eq!(crate::index::oplog::read_committed_seq(&tenant_path), 0);

        let object_id = kind.object_id();
        let expected_object = expected_fixture_document_json(kind);
        let expected_document = Document::from_json(&expected_object).unwrap();
        ...
        assert_eq!(crate::index::oplog::read_committed_seq(&tenant_path), 1);
        let durable_version = IndexManager::new(base)
            .get_object_version(tenant, object_id)
            .unwrap()
            .unwrap_or_else(|| panic!("{tenant}/{object_id} should have a durable version"));
        assert_eq!(durable_version.origin_seq, Some(1));
        assert_eq!(
            durable_version.effect_digest,
            Some(crate::index::oplog::upsert_effect_digest(
                &expected_document
            ))
        );
    }
...
fn assert_fixture_generation_reopenable(
    ...
    assert_reopenable_fixture_tree(
        ...
        tenant,
        kind.object_id(),
        expected_fixture_document_json(kind),
        kind.generation(),
        &[kind.object_id()],
    );
}

fn expected_fixture_document_json(kind: FixtureTreeKind) -> serde_json::Value {
    serde_json::json!({
        "_id": kind.object_id(),
        "objectID": kind.object_id(),
        "title": kind.title(),
