...
async fn replication_ack_exact_settings_invalidate_cached_embedder() {
    ...
    let mut initial_settings = IndexSettings::default();
    initial_settings.embedders = Some(std::collections::HashMap::from([(
        "default".to_string(),
...
#[tokio::test]
async fn olr_replication_ack_refuses_unproven_copy_and_clear_before_effects() {
...
#[tokio::test]
async fn replication_ack_preserves_upsert_then_clear_index_order() {
    let tmp = TempDir::new().unwrap();
    let manager = IndexManager::new(tmp.path());
    let ops = vec![
        make_upsert_op(54, 1_000, "node-a", "t1", "doc1", "MustBeCleared"),
        make_index_op(
            55,
            2_000,
            "node-a",
            "t1",
            "clear_index",
            serde_json::json!({"index_name": "t1"}),
        ),
    ];

    let acked_seq = apply_ops_to_manager(&manager, "t1", &ops)
        .await
        .expect("upsert followed by clear is a valid ordered batch");

    assert_eq!(acked_seq, 55);
    assert!(
        manager.get_document("t1", "doc1").unwrap().is_none(),
        "the staged upsert must flush before the later clear operation"
    );
}
