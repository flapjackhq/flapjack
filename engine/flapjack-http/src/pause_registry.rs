...
impl GlobalMutationFence {
    pub fn open(data_root: &Path) -> std::io::Result<Self> {
        ...
        let state_file = if canonical_data_root
            .file_name()
            .and_then(|name| name.to_str())
            == Some("data")
        {
            GLOBAL_FENCE_FILE.to_string()
        } else {
            let name = canonical_data_root
                .file_name()
                .and_then(|name| name.to_str())
                .ok_or_else(|| {
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        "Flapjack data root name is not valid UTF-8",
                    )
                })?;
            format!(".{name}.{GLOBAL_FENCE_FILE}")
        };
