...
pub async fn serve() -> Result<(), Box<dyn std::error::Error>> {
    ...
    enforce_startup_auth_policy(&server_config, &node_config);
...
fn enforce_startup_auth_policy(
...
fn flush_and_log_analytics_before_shutdown(
    analytics_enabled: bool,
    collector: &flapjack::analytics::AnalyticsCollector,
) {
    flush_analytics_before_shutdown(analytics_enabled, collector);
    if analytics_enabled {
        tracing::info!("[shutdown] Analytics buffers flushed");
    }
}

fn log_shutdown_outcome(outcome: &ShutdownWaitOutcome, shutdown_timeout_secs: u64) {
    match outcome {
        ...
        ShutdownWaitOutcome::Failed(error) => {
            tracing::error!(
                error,
...
async fn run_graceful_shutdown(
    ...
    let outcome = full_graceful_shutdown_with_migrations(
        shutdown_timeout_secs,
        move || {
            flush_and_log_analytics_before_shutdown(analytics_enabled, &analytics_collector);
        },
    ...
    log_shutdown_outcome(&outcome, shutdown_timeout_secs);
