#!/bin/bash
# Start the stable flapjack-server binary for dashboard development.
...
# Rebuild: cargo build -p flapjack-server --release && cp target/release/flapjack-server dashboard/bin/flapjack-server-stable

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BIN="$SCRIPT_DIR/../bin/flapjack-server-stable"

if [ ! -f "$BIN" ]; then
  echo "Stable binary not found at $BIN"
  echo "Build it with: cargo build -p flapjack-server --release && cp target/release/flapjack-server dashboard/bin/flapjack-server-stable"
  exit 1
fi
...
echo "Starting stable flapjack-server on $FLAPJACK_BIND_ADDR"
